        ...
        <form method="POST" action="{{ route("admin.comments.update", [$comment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('submit') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="submit" id="submit" value="1" {{ $comment->submit || old('submit', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('document') ? 'is-invalid' : '' }}" name="document_id" id="document_id" required>
                    @foreach($documents as $id => $document)
                        <option value="{{ $id }}" {{ (old('document_id') ? old('document_id') : $comment->document->id ?? '') == $id ? 'selected' : '' }}>{{ $document }}</option>
                    @endforeach
                </select>
