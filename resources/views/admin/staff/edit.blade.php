    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.staff.update", [$staff->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('staff_code') ? 'is-invalid' : '' }}" type="text" name="staff_code" id="staff_code" value="{{ old('staff_code', $staff->staff_code) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_kh') ? 'is-invalid' : '' }}" type="text" name="name_kh" id="name_kh" value="{{ old('name_kh', $staff->name_kh) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender" id="gender">
                    ...
                    @foreach(App\Models\Staff::GENDER_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gender', $staff->gender) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('title') ? 'is-invalid' : '' }}" name="title_id" id="title_id">
                    @foreach($titles as $id => $title)
                        <option value="{{ $id }}" {{ (old('title_id') ? old('title_id') : $staff->title->id ?? '') == $id ? 'selected' : '' }}>{{ $title }}</option>
                    @endforeach
                </select>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('department') ? 'is-invalid' : '' }}" name="department_id" id="department_id">
                    @foreach($departments as $id => $department)
                        <option value="{{ $id }}" {{ (old('department_id') ? old('department_id') : $staff->department->id ?? '') == $id ? 'selected' : '' }}>{{ $department }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('position') ? 'is-invalid' : '' }}" name="position_id" id="position_id">
                    @foreach($positions as $id => $position)
                        <option value="{{ $id }}" {{ (old('position_id') ? old('position_id') : $staff->position->id ?? '') == $id ? 'selected' : '' }}>{{ $position }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('dob') ? 'is-invalid' : '' }}" type="text" name="dob" id="dob" value="{{ old('dob', $staff->dob) }}">
                @if($errors->has('dob'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $staff->phone) }}">
                @if($errors->has('phone'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $staff->email) }}">
                @if($errors->has('email'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ $staff->is_active || old('is_active', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('staff_position') ? 'is-invalid' : '' }}" type="text" name="staff_position" id="staff_position" value="{{ old('staff_position', $staff->staff_position) }}">
